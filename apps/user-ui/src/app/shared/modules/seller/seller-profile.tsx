    ...
    const {data:products,isLoading} = useQuery({
        ...
        queryFn:async()=>{
            const res = await axiosInstance.get(
                `seller/api/get-seller-product/${shop?.id}?page=1&limit=10`
            );
    ...
    useEffect(()=>{
        const fetchFollowStatus = async()=>{
            ...
            try {
                ...
                setIsFollowing(res.data.setIsFollowing!==null);
            } catch (error) {
    ...
    return (
        <div>
            <div className="relative w-full flex justify-center">
                ...
                alt="Seller Cover"
                className="w-full h-[h-400px] object-cover"
                width={1200}
                height={300}
                />
            </div>

            <div className="w-[85%] lg:w-[70%] mt-[-50px] mx-auto realtive z-20 flex flex-col lg:flex-row gap-6">
                <div className="bg-gray-200 p-6 rounded-lg shadow-lg flex-1">
                    <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                        <div className="relative w-[100px] h-[100px] rounded-full border-4 border-slate-300 overflow-hidden">
                            <Image
                            src={
                                shop?.avatar || "store_fallback.png"
                            }
                            alt="seller avatar"
                            layout="fill"
                            objectFit="cover"
                        ...
                        <button
                        className={`px-6 py-2 h-[40px] rounded-lg font-semibold fle items-center gap-2 transition ${
                            isFollowing
                            ? "bg-red-500 hover:bg-red-600"
                            : "bg-biue-600 hover:bg-blue-700"
                        }`}
